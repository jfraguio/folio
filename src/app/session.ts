...
/**
 * Abre una novela. Devuelve `null` si el usuario decide no abrirla (p. ej. está en otra pestaña
...
export async function startSession(o: SessionOptions): Promise<Session | null> {
  ...
  if (!lock.acquired) {
    ...
    if (result === 'refused') {
      lock.release();
      notice('La otra pestaña tiene cambios que aún no ha podido guardar. Resuélvelo allí antes de abrir la novela aquí.', 8000);
      o.onExit();
      return null;
    }
    ...
    if (result === 'no-response') {
      notice('La otra pestaña no responde. Se abre aquí; si allí sigue abierta, Folio avisará de cualquier conflicto al guardar.', 8000);
    }
  }
  ...
  try {
    ...
    // detecta la diferencia y lo escribe de verdad (antes se daba por guardado sin tocar el archivo).

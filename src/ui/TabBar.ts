 ...
 * el número que haya.
 *
 * Tras las tabs principales, separadas por una barra vertical y con fondo más oscuro, van las subtabs de
...
export class TabBar {
  ...
  /**
   * Barra vertical entre las tabs principales y las subtabs. Siempre está en la tira (marca dónde
   * acaban las principales) y solo se ve si hay subtabs. Es decorativa: fuera del árbol accesible.
   */
  private readonly divider: HTMLElement;

  constructor(private readonly o: TabBarOptions) {
    this.divider = el('span', { class: 'tab-bar__divider', attrs: { 'aria-hidden': 'true' } });
    this.strip = el('div', { class: 'tab-bar__strip', attrs: { role: 'tablist' } }, this.divider);
  ...
  render(): void {
    ...
    for (let k = 0; k < count; k++) {
      ...
      // Con una subtab abierta, su tab madre lleva el fondo de las subtabs: así se ve de qué tab son.
      this.buttons[k]!.classList.toggle('tab-bar__tab--parent', k === this.activeTab && this.activeSub >= 0);
    }
   ...
   * principal justo antes del separador (que precede a la primera subtab).
   */
  private fit(list: HTMLButtonElement[], count: number, select: (k: number) => void, className: string): void {
    while (list.length < count) {
      ...
      list.push(btn);
      if (list === this.buttons) this.strip.insertBefore(btn, this.divider);

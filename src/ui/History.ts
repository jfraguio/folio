...
/**
 * Historial: copias de seguridad de apertura del archivo (ver persistence/backups.ts).
 * Solo lista y descarga. A propósito no hay «restaurar»: una copia nunca vuelve al editor ni al .md.
 */
export function openHistory(todoId: string, fileName: string, restoreFocus?: () => void): void {
  ...
  void listBackups(todoId).then(
    (backups) => {
      clear(list);
      header.textContent = backups.length === 1 ? 'Historial · 1 copia' : `Historial · ${backups.length} copias`;
      if (!backups.length) {
        list.appendChild(el('span', { class: 'panel__meta' }, 'Todavía no hay copias de este archivo.'));
        return;
      }
      for (const b of backups) {
        const name = backupFileName(fileName, b);
        list.appendChild(
          el(
            'div',
            { class: 'history__row' },

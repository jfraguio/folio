...
export class LiveDraft {
  ...
  async flush(): Promise<void> {
    ...
    try {
      ...
      await db.put('drafts', { todoId: this.todoId, ts: Date.now(), text });
    } catch {
  ...
  async clear(): Promise<void> {
    ...
    this.pending = null;
    const db = await getDB();
    await db.delete('drafts', this.todoId);
  }

  static async read(todoId: string): Promise<DraftRecord | undefined> {
    const db = await getDB();
    return db.get('drafts', todoId);
  }
}

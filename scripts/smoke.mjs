...
const dotDirty = await page.$eval('.panel__actions .status-dot', (e) => e.dataset.state);
check('notas: el punto refleja cambios sin guardar', dotDirty === 'dirty' || dotDirty === 'saving', dotDirty);
await page.waitForFunction(() => document.querySelector('.panel__actions .status-dot')?.dataset.state === 'saved', null, { timeout: 5000 }).catch(() => null);

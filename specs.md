...
- **Dónde se ven**: en la misma barra, **después de todas las tabs principales**, separadas por una barra vertical fina y tenue (`.tab-bar__divider`, 1px, color `--fg-dim` atenuado; oculta en modo zen) y con un fondo ligeramente más oscuro (`--tab-sub-bg`; la activa, `--tab-sub-bg-active`). Solo se muestran las subtabs de la tab abierta: al cambiar de tab principal cambian las subtabs visibles. Con una subtab abierta, su tab madre lleva el mismo fondo que las subtabs (`.tab-bar__tab--parent`), para ver de qué tab son. Van detrás (y no intercaladas tras su madre) para que las tabs principales no se muevan de sitio al cambiar de tab.
